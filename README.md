...
Clone the repo and point Gradle at your Android SDK by creating `local.properties` in the project root (this file is gitignored):
...
The APK lands at `app/build/outputs/apk/debug/app-debug.apk`. Enable USB debugging on the phone (Settings → System → Developer options → USB debugging), plug it in, and install:
...
2. Tap **Test fetch now** to confirm it prints your current `5H` and `1W` percentages, which should match `claude.ai/settings/usage`.
3. Tap **Disable battery optimization** and allow it, so Android doesn't kill the 15-minute background refresh.
4. Long-press your home screen → **Widgets** → **Claude Usage**, drag it on, and resize it however you like. Tap it any time to refresh immediately.

## Project layout

The networking and auth core lives in `app/src/main/java/com/usage/claudewidget/data` (endpoint constants, encrypted storage, cookie harvesting, the Cloudflare resolver, and the repository that orchestrates fetch → Cloudflare retry → auth-state transitions). The WebView login is in `auth/LoginActivity.kt`, the setup/debug screen is in `ui/MainActivity.kt`, the Glance widget and its layouts are in `widget/`, and the background refresh scheduling is in `work/`.
